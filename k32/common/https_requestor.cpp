...
struct Implementation
  {
    ...
    // requests
    wkptr<::poseidon::HTTPS_Client_Session> weak_session;
...
POSEIDON_HIDDEN_X_STRUCT(HTTPS_Requestor,
    Implementation);
...
enqueue(const shptr<HTTP_Future>& req)
  {
    ...
    if(!session) {
      session = this->m_impl->https_client.connect(
      ...
      POSEIDON_LOG_INFO(("Connecting to `https://$1`"), session->https_default_host());
    }
    ...
    session->https_request(move(ht_req), req->m_req_payload);
  }
