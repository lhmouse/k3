...
#ifndef K32_COMMON_HTTPS_REQUESTOR_
#define K32_COMMON_HTTPS_REQUESTOR_

#include "../fwd.hpp"
#include "http_future.hpp"
namespace k32 {

class HTTPS_Requestor
  {
  private:
  ...
  public:
    HTTPS_Requestor(const HTTPS_Requestor&) = delete;
    HTTPS_Requestor& operator=(const HTTPS_Requestor&) & = delete;
    ~HTTPS_Requestor();

    // Sets the target host. This must be called before any request messages
    // can be sent.
    void
    set_target_host(const cow_string& host, uint16_t port = 443);
